...
- 画面名: `施設管理者ログイン`
- パス: `/facility/login`
- URL: https://carebase-admin.vercel.app/facility/login

## 概要

施設管理者ログイン画面設計書です。
メールアドレスとパスワードによる認証機能を提供し、認証成功時は施設管理者管理画面のTOP（事業所一覧）へ遷移します。

認証フローは以下の通りです：

1. メールアドレスとパスワードで認証
2. 認証成功時にトークンとユーザー情報を取得
3. 施設管理者管理画面TOP (`/facility`) へ自動遷移
...
施設管理者向けログイン画面は、CareBaseロゴ、ログインフォーム、フッターで構成されます。
...
| ---------------------------- | --------------- | ---- | -------- | -------- | ------------ | ---------------------------------------------------------- | -------------------------------------------------------- |
| ページタイトル               | CardTitle       | -    | -        | -        | -            | 施設管理者 ログイン                                        | カード内ヘッダー                                         |
...
| ---------------------- | -------------------------------------- | -------------------------- | ---------------------------------------------------------- |
| ログインボタン         | メールアドレスとパスワードで認証を実行 | `/api/v1/auth/admin/login` | 施設管理者管理画面TOP (`/facility`)                        |
| フォーム送信           | Enterキー押下時の認証実行              | 同上                       | 同上                                                       |
| パスワードリマインダー | パスワードリセット画面へ遷移           | -                          | パスワードリマインダー画面 (`/facility/password-reminder`) |
...
- **LoginForm** (`components/2_molecules/login-form.tsx`)
  - フォーム全体の制御とレイアウト
  - roleName="施設管理者", redirectPath="/facility" で設定
...
| エンドポイント                         | 説明                   |
| -------------------------------------- | ---------------------- |
| `/api/v1/auth/admin/login`             | 施設管理者ログイン     |
