...
- 画面名: `施設管理者パスワードリセット`
- パス: `/facility/password-reset`
- URL: https://carebase-admin.vercel.app/facility/password-reset

## 概要

パスワードリマインダー画面設計書です。
パスワードを忘れた施設管理者が、メールアドレスを入力してパスワードリセット機能を利用できる画面を提供します。
施設管理者として登録されているメールアドレスの確認を行い、成功時にはメール送信と成功画面への遷移を実装します。

認証フローは以下の通りです：

1. メールアドレスを入力してリマインダーを要求
2. 施設管理者として登録されているメールアドレスかチェック
...
| -------------------- | --------------- | ---- | -------- | -------- | ------------ | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| ページタイトル       | CardTitle       | -    | -        | -        | -            | 施設管理者パスワードリセット                                                                         | カード内ヘッダー                                     |
...
| -------------- | --------------------------------- | -------------------------------------------- | ------------------------------------------ |
| 送信ボタン     | メールアドレスでリマインダー要求  | `/api/v1/auth/admin/facility/password-reset` | 成功画面表示（同一画面内）                 |
| フォーム送信   | Enterキー押下時のリマインダー実行 | 同上                                         | 同上                                       |
| ログインへ戻る | ログイン画面へ遷移                | -                                            | 施設管理者ログイン画面 (`/facility/login`) |
...
| フォーム全体   | submit     | 施設管理者登録チェック | 施設管理者として登録されていないメールアドレスです |
...
| エンドポイント                                  | 説明                   |
| ----------------------------------------------- | ---------------------- |
| `/api/v1/auth/admin/facility/password-reminder` | パスワードリマインダー |
...
- [ログイン画面設計書](../facility/login/README.md)
