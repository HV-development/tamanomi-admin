...
function validateForm(data: CampaignFormData): CampaignFormErrors {
  ...
  if (!data.code.trim()) {
    ...
    errors.code = '4〜20文字の英小文字・半角数字で入力してください';
  } else if (data.code.length < 4 || data.code.length > 20) {
    errors.code = '4〜20文字の英小文字・半角数字で入力してください';
  }
...
export default function NewCampaignPage() {
  ...
  const proceedToConfirm = useCallback((acknowledgedOverlapIds: string[]) => {
    if (acknowledgedOverlapIds.length > 0) {
      sessionStorage.setItem('campaignAcknowledgedOverlapIds', JSON.stringify(acknowledgedOverlapIds));
    } else {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const overlapRes = await fetch(`/api/admin/campaigns/check-overlap?${params.toString()}`, {
        method: 'GET',
        cache: 'no-store',
      });
      if (!overlapRes.ok) {
      ...
      const overlapData = (await overlapRes.json()) as { overlappingCampaigns?: Campaign[] };
      overlaps = overlapData.overlappingCampaigns ?? [];
    } catch {
      showError('期間重複チェックに失敗しました。時間をおいて再度お試しください');
      return;
    } finally {
    ...
    proceedToConfirm([]);
  }, [formData, showError, proceedToConfirm]);
